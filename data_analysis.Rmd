...
ggplot(data = table_Obs_D1_Date, mapping = aes(x = RecordedDate, y = count)) +
  geom_line() +
  labs(title = "Number of daily observations over time",
       caption = "Data aggregated per day; Observation period May 20, 2021 to June 25, 2022",
       x = "Date",
       y = "Number of daily observations") +
  ylim(0, 2000) +
  theme(text = element_text(size = 15))
...
D1_gen_restr_lineplot <- micro_restr %>% select(D1, bl_mn_idx_t) %>% ungroup() %>%
  ...
  labs(title = "Relative frequency of D1 (anxiety) categories across general containment measures index",
       x = "General containment measures index",
       y = "Relative frequency",
       color = "D1/D2 (anxiety/depression)") +
  theme(text = element_text(size = 15),
        legend.position = "bottom")
...
ggarrange(D1_gen_restr_lineplot, D2_gen_restr_lineplot, ncol = 1, nrow = 2,
  ...
  annotate_figure(plot, bottom = text_grob("Data aggregated per day: Observation period May 20, 2021 to June 25, 2022; General containment measuresindex is rounded to the nearest integer\nObservations with at least 10 responses for each mental health category across index scores.", 
               color = "black", size = 12))
ggsave("Plots/presentation/arranged_D1_D2_gen_measures.png", width = 15, height = 13)
```
