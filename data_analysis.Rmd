...
measures_daycare_date <- ggplot(data = bl_massnahmen_index_tag, 
  ...
  geom_line(aes(alpha = alpha_values)) +
  geom_vline(xintercept = c(as.Date("2022-06-25"), as.Date("2021-05-20")), linetype="dashed") +
  labs(title = "German daycare center measures index\nover time per federal states",
       x = "Date",
...
measures_sec_schools_date <- ggplot(data = bl_massnahmen_index_tag, 
  ...
  geom_line(aes(alpha = alpha_values)) +
  geom_vline(xintercept = c(as.Date("2022-06-25"), as.Date("2021-05-20")), linetype="dashed") +
  labs(title = "German secondary school measures index\nover time per federal states",
       x = "Date",
...
measures_elem_schools_date <- ggplot(data = bl_massnahmen_index_tag, 
  ...
  geom_line(aes(alpha = alpha_values)) +
  geom_vline(xintercept = c(as.Date("2022-06-25"), as.Date("2021-05-20")), linetype="dashed") +
  labs(title = "Elementary school measures index\nover time per federal states",
       x = "Date",
...
# Plot: D1 vs German restriction index
gen_restr_count_D1 <- micro_restr %>% select(D1, bl_mn_idx_t) %>% ungroup() %>%
...
D1_gen_restr_lineplot <- micro_restr %>% select(D1, bl_mn_idx_t) %>% ungroup() %>%
    ...
    group_by(bl_mn_idx_t) %>%
    filter(all(n >= 10)) %>%
    ggplot(aes(x = bl_mn_idx_t)) +
    geom_line(aes(color = D1, y = perc)) +
  geom_point(size = 0.75, aes(color = D1, y = perc)) +
  geom_rug(data = gen_restr_count_D1, sides="b", mapping = aes(linewidth = n)) +
...
# Plot: D2 vs German restriction index
gen_restr_count_D2 <- micro_restr %>% select(D2, bl_mn_idx_t) %>% ungroup() %>%
    mutate(bl_mn_idx_t = round(bl_mn_idx_t)) %>%
...
D2_gen_restr_lineplot <- micro_restr %>% select(D2, bl_mn_idx_t) %>% ungroup() %>%
    ...
    filter(all(n >= 10)) %>%
    ggplot(aes(x = bl_mn_idx_t)) +
    geom_line(aes(color = D2, y = perc)) +
  geom_point(size = 0.75, aes(color = D2, y = perc)) +
  geom_rug(data = gen_restr_count_D2, sides="b", mapping = aes(linewidth = n)) +
...
ggarrange(D1_gen_restr_lineplot, D2_gen_restr_lineplot, ncol = 1, nrow = 2,
          labels = c("A", "B", "C", "D"), common.legend = TRUE, legend = "bottom",
          font.label=list(color="black",size=15))
ggsave("Plots/paper/arranged_D1_D2_gen_measures.png", width = 13, height = 13)

## -----------

# Plot: D1 vs German restriction index vs E4
gen_restr_count_D1_E4 <- micro_restr %>% select(D1, bl_mn_idx_t, E4) %>% ungroup() %>%
    mutate(bl_mn_idx_t = round(bl_mn_idx_t)) %>%
...
micro_restr %>% select(D1, bl_mn_idx_t, E4) %>% ungroup() %>%
    ...
    filter(all(n >= 3),
           length(unique(D1)) == 5) %>%
    ggplot(aes(x = bl_mn_idx_t)) +
    geom_line(aes(y = perc, color = D1)) +
  geom_point(size = 0.75, aes(y = perc, color = D1)) +
  geom_rug(data = gen_restr_count_D1_E4, sides="b", mapping = aes(linewidth = n)) +
    facet_wrap(~E4) +
...
# Plot: D2 vs German restriction index vs E4
gen_restr_count_D2_E4 <- micro_restr %>% select(D2, bl_mn_idx_t, E4) %>% ungroup() %>%
    mutate(bl_mn_idx_t = round(bl_mn_idx_t)) %>%
...
micro_restr %>% select(D2, bl_mn_idx_t, E4) %>% ungroup() %>%
    ...
    filter(all(n >= 3),
           length(unique(D2)) == 5) %>%
    ggplot(aes(x = bl_mn_idx_t)) +
    geom_line(aes(y = perc, color = D2)) +
  geom_point(size = 0.75, aes(y = perc, color = D2)) +
  geom_rug(data = gen_restr_count_D1_E4, sides="b", mapping = aes(linewidth = n)) +
    facet_wrap(~E4) +
  labs(x = "General containment measures index",
       y = "Relative frequency",
...
# Plot: D1 vs German restriction index vs E8
gen_restr_count_D1_E8 <- micro_restr %>% select(D1, bl_mn_idx_t, E8) %>% ungroup() %>%
    mutate(bl_mn_idx_t = round(bl_mn_idx_t)) %>%
...
micro_restr %>% select(D1, bl_mn_idx_t, E8) %>% ungroup() %>%
    ...
    geom_line(aes(y = perc, color = D1)) +
  geom_point(size = 0.75, aes(y = perc, color = D1)) +
  geom_rug(data = gen_restr_count_D1_E8, sides="b", mapping = aes(linewidth = n)) +
    facet_wrap(~E8) +
  labs(x = "General containment measures index",
       y = "Relative frequency",
       color = "D1 (anxiety)") +
...
# Plot: D2 vs German restriction index vs E8
gen_restr_count_D2_E8 <- micro_restr %>% select(D2, bl_mn_idx_t, E8) %>% ungroup() %>%
    mutate(bl_mn_idx_t = round(bl_mn_idx_t)) %>%
...
micro_restr %>% select(D2, bl_mn_idx_t, E8) %>% ungroup() %>%
    ...
    geom_line(aes(y = perc, color = D2)) +
  geom_point(size = 0.75, aes(y = perc, color = D2)) +
  geom_rug(data = gen_restr_count_D2_E8, sides="b", mapping = aes(linewidth = n)) +
    facet_wrap(~E8) +
  labs(x = "General containment measures index",
       y = "Relative frequency",
       color = "D2 (depression)") +
...
# Plot: D1 vs German restriction index vs E3
gen_restr_count_D1_E3 <- micro_restr %>% select(D1, bl_mn_idx_t, E3) %>% ungroup() %>%
    mutate(bl_mn_idx_t = round(bl_mn_idx_t)) %>%
...
micro_restr %>% select(D1, bl_mn_idx_t, E3) %>% ungroup() %>%
    ...
    geom_line(aes(y = perc, color = D1)) +
  geom_point(size = 0.75, aes(y = perc, color = D1)) +
  geom_rug(data = gen_restr_count_D1_E3, sides="b", mapping = aes(linewidth = n)) +
    facet_wrap(~E3) +
  labs(x = "General containment measures index",
       y = "Relative frequency",
       color = "D1 (anxiety)") +
...
# Plot: D2 vs German restriction index vs E3
gen_restr_count_D2_E3 <- micro_restr %>% select(D2, bl_mn_idx_t, E3) %>% ungroup() %>%
    mutate(bl_mn_idx_t = round(bl_mn_idx_t)) %>%
...
micro_restr %>% select(D2, bl_mn_idx_t, E3) %>% ungroup() %>%
    ...
    geom_line(aes(y = perc, color = D2)) +
  geom_point(size = 0.75, aes(y = perc, color = D2)) +
  geom_rug(data = gen_restr_count_D2_E3, sides="b", mapping = aes(linewidth = n)) +
    facet_wrap(~E3) +
  labs(x = "General containment measures index",
       y = "Relative frequency",
       color = "D2 (depression)") +
...
# Plot: D1 vs German restriction index vs E2
gen_restr_count_D1_E2 <- micro_restr %>% select(D1, bl_mn_idx_t, E2) %>% ungroup() %>%
    mutate(bl_mn_idx_t = round(bl_mn_idx_t)) %>%
...
micro_restr %>% select(D1, bl_mn_idx_t, E2) %>% ungroup() %>%
    ...
    geom_line(aes(y = perc, color = D1)) +
  geom_point(size = 0.75, aes(y = perc, color = D1)) +
  geom_rug(data = gen_restr_count_D1_E2, sides="b", mapping = aes(linewidth = n)) +
    facet_wrap(~E2) +
  labs(x = "General containment measures index",
       y = "Relative frequency",
       color = "D1 (anxiety)") +
...
# Plot: D2 vs German restriction index vs E2
gen_restr_count_D2_E2 <- micro_restr %>% select(D2, bl_mn_idx_t, E2) %>% ungroup() %>%
    mutate(bl_mn_idx_t = round(bl_mn_idx_t)) %>%
...
micro_restr %>% select(D2, bl_mn_idx_t, E2) %>% ungroup() %>%
    ...
    geom_line(aes(y = perc, color = D2)) +
  geom_point(size = 0.75, aes(y = perc, color = D2)) +
  geom_rug(data = gen_restr_count_D2_E2, sides="b", mapping = aes(linewidth = n)) +
    facet_wrap(~E2) +
  labs(x = "General containment measures index",
       y = "Relative frequency",
       color = "D2 (depression)") +
...
# Plot: D1 vs German restriction index vs D7a
gen_restr_count_D1_D7a <- micro_restr %>% select(D1, bl_mn_idx_t, D7a) %>% ungroup() %>%
    mutate(bl_mn_idx_t = round(bl_mn_idx_t)) %>%
...
micro_restr %>% select(D1, bl_mn_idx_t, D7a) %>% ungroup() %>%
    ...
    geom_line(aes(y = perc, color = D1)) +
  geom_point(size = 0.75, aes(y = perc, color = D1)) +
  geom_rug(data = gen_restr_count_D1_D7a, sides="b", mapping = aes(linewidth = n)) +
    facet_wrap(~D7a) +
  labs(x = "General containment measures index",
       y = "Relative frequency",
       color = "D1 (anxiety)") +
...
# Plot: D2 vs German restriction index vs D7a
gen_restr_count_D2_D7a <- micro_restr %>% select(D2, bl_mn_idx_t, D7a) %>% ungroup() %>%
    mutate(bl_mn_idx_t = round(bl_mn_idx_t)) %>%
...
ggplot(data = bl_massnahmen_index_tag, mapping = aes(x = datum, y = bl_mn_idx_t, color = bundesland)) +
  geom_line(aes(alpha = alpha_values)) +
  geom_vline(xintercept = c(as.Date("2022-06-25"), as.Date("2021-05-20")), linetype="dashed") +
  labs(title = "German general containment measures index over time per federal states",
       caption = "Data aggregated per day: Relevant observation period May 20, 2021 to June 25, 2022",
       x = "Date",
       y = "General containment measures index", 
       color = "Federal state") +
    scale_alpha_continuous(guide="none") +
  theme(text = element_text(size = 15))
ggsave(filename = "Plots/presentation/gen_measures_date.png", width = 12, height = 9)
...
ggarrange(Obs_RecordedDate_D1_lineplot, Obs_RecordedDate_D2_lineplot, ncol = 1, nrow = 2,
          labels = c("A", "B", "C", "D")) |>
  annotate_figure(plot, top = text_grob("Observations of D1 (anxiety) and D2 (depression) over time", 
               color = "black", face = "bold", size = 20)) |>
  annotate_figure(plot, bottom = text_grob("Daily aggregated data: Observation period May 20, 2021 to June 25, 2022", 
               color = "black", size = 12))
ggsave("Plots/presentation/arranged_RecordedDate_D1_D2.png", width = 12, height = 9)
...
ggarrange(D1_gen_restr_lineplot, D2_gen_restr_lineplot, ncol = 1, nrow = 2,
          labels = c("A", "B", "C", "D"), common.legend = TRUE, legend = "bottom",
          font.label=list(color="black",size=15)) |>
  annotate_figure(plot, top = text_grob("Relative frequency of mental health categories across general containment measures index", 
               color = "black", face = "bold", size = 20)) |>
  annotate_figure(plot, bottom = text_grob("Data aggregated per day: Observation period May 20, 2021 to June 25, 2022; General containment measuresindex is rounded to the nearest integer\nObservations with at least 10 responses for each mental health category across index scores.", 
               color = "black", size = 12))
ggsave("Plots/presentation/arranged_D1_D2_gen_measures.png", width = 13, height = 13)
```
